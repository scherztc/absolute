...
describe 'Creating a Video' do
  ...
  describe 'with a related link' do
    it "should allow me to attach the link on the create page" do
      ...
      classify_what_you_are_uploading 'Video'
      within '#new_video' do
      ...
      expect(page).to have_selector('h1', text: 'Video')
...
describe 'An existing Video owned by the user' do
  let(:user) { FactoryGirl.create(:user) }
  let(:work) { FactoryGirl.create(:video, user: user) }
...
describe 'Viewing a Video that is private' do
  let(:user) { FactoryGirl.create(:user) }
  ...
  it 'should show a stub indicating we have the work, but it is private' do
    ...
    expect(page).to have_content('The video you have tried to access is private')
